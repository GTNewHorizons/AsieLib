...
public abstract class TweakBaseConfig extends TweakBase {
    ...
    public boolean onRecipe(List recipeList, IRecipe recipe) {
        return false;
        /*
         * for(String s: types) { // TODO //ItemStack is = CrossModUtils.getItemStackFromConfig(s.split(",")[0],
         * s.split(",")[1], s.split(",")[2], 1, 0, s.split(",")[1].equals("item")); String meta = s.split(",")[3];
         * if(meta.equals("all")) { if(CraftingTweaker.removeOutputRecipe(recipeList, recipe, is, true)) return true; }
         * else { is.setItemDamage(new Integer(meta).intValue()); if(CraftingTweaker.removeOutputRecipe(recipeList,
         * recipe, is, false)) return true; } } return false;
         */
    }
    ...
    private ItemStack getItemStack(String mod, String category, String name, int stackSize, int metadata) {
        return null;
        // TODO
        // return CrossModUtils.getItemStackFromConfig(mod, category, name, stackSize, metadata,
