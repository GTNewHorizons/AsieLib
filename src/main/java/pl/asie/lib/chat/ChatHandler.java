...
public class ChatHandler {
    ...
    public ChatHandler(Configuration config) {
        ...
        enableGreentext = config.get("chat", "enableGreentext", false, ">implying anyone will ever turn this on")
                .getBoolean(false);
        nickLevel = config.get(
                "chat",
                "nicknamesForEveryone",
                true,
                "Disable to make changing the own nickname require Op rights on a server").getBoolean(true) ? 0 : 2;
        realnameLevel = config
                .get(
                        "chat",
                        "realnamesForEveryone",
                        true,
        ...
        messageFormat = config
                ...
                .getString();
        enableChatLog = config.get("chat", "chatLog", true, "Enable this to log server chat to console")
                .getBoolean(true);
    ...
    public void chatEvent(ServerChatEvent event) {
        ...
        try {
            formattedMessage = formattedMessage.replaceAll("%u", username).replaceAll("%m", message)
                    .replaceAll("%w", event.player.worldObj.provider.getDimensionName())
                    .replaceAll("%H", pad(now.get(Calendar.HOUR_OF_DAY)))
                    .replaceAll("%M", pad(now.get(Calendar.MINUTE))).replaceAll("%S", pad(now.get(Calendar.SECOND)));
        } catch (Exception e) {
            e.printStackTrace();
            formattedMessage = EnumChatFormatting.RESET + "<"
                    + username
                    + ""
        ...
        for (WorldServer ws : MinecraftServer.getServer().worldServers) {
            for (Object o : ws.playerEntities) {
                if (o instanceof EntityPlayer) {
                    ...
                    if ((!useRadius || event.player == target
                            || event.player.getDistanceToEntity(target) <= CHAT_RADIUS)
