...
public class RayTracer {
    ...
    protected MovingObjectPosition rayTrace(EntityLivingBase entity, double distance) {
        ...
        for (double i = 1.0; i < distance; i += 0.2) {
            ...
            MovingObjectPosition blockCheck = entity.worldObj.rayTraceBlocks(
                    Vec3.createVectorHelper(position.xCoord, position.yCoord, position.zCoord),
                    search,
                    false);
            if (blockCheck != null && blockCheck.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                /*
                 * double d1 = position.squareDistanceTo(blockCheck.hitVec); double d2 =
                 * position.squareDistanceTo(search);
                 */
                if (position.squareDistanceTo(blockCheck.hitVec) < position.squareDistanceTo(search)) {
                    return blockCheck;
                }
            }
    ...
    protected Entity getEntity(EntityLivingBase base, Vec3 position, Vec3 search, Vec3 look, AxisAlignedBB searchBox,
            double v) {
        ...
        if (entityList.size() > 1) {
            ...
            /*
             * Vec3 newSearch = search.addVector(-v / 2.0 * look.xCoord, -v / 2.0 * look.yCoord, -v / 2.0 *
             * look.zCoord); AxisAlignedBB newSearchBox = AxisAlignedBB.getBoundingBox( newSearch.xCoord - v / 2.0,
             * newSearch.yCoord - v / 2.0, newSearch.zCoord - v / 2.0, newSearch.xCoord + v / 2.0, newSearch.yCoord + v
             * / 2.0, newSearch.zCoord + v / 2.0); return getEntity(world, newSearch, look, newSearchBox, v / 2.0);
             */
        } else {
