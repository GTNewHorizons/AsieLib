/*
 * DFPWM implementation in Java operates on 8-bit signed PCM data and little-endian DFPWM data by Ben "GreaseMonkey"
 * Russell, 2013 - Public Domain NOTE, len is in bytes relative to DFPWM (len*8 PCM bytes) also the main() function
 * takes unsigned 8-bit data and converts it to suit
 */
...
public class DFPWM {
    ...
    private void ctx_update(boolean curbit) {
        ...
        if (nlevel == level && level != target) nlevel += ((target == 127) ? 1 : -1);
        ...
        if (nresponse == response && response != rtarget) nresponse += (rtarget == 255 ? 1 : -1);
    ...
    public void decompress(byte[] dest, byte[] src, int destoffs, int srcoffs, int len) {
        for (int i = 0; i < len; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                // apply noise shaping
                int blevel = (curbit == lastbit ? level : ((flastlevel + level) >> 1));
                flastlevel = level;
